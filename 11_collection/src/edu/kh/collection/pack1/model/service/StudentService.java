...
//	private List<Student> studentList = new LinkedList<Student>(); // 추가, 수정, 삭제에 효율적
	...
	public StudentService() { // 기본생성자
		...
		studentList.add(new Student("고길동", 23, "서울시 종로구", 'M', 100));
		studentList.add(new Student("마이콜", 26, "서울시 행복구", 'M', 85));
		studentList.add(new Student("고희동", 20, "서울시 고백구", 'F', 90));
		studentList.add(new Student("또치", 21, "서울시 사랑구", 'F', 80));
	 ...
	 *   
	 * - 수정이 완료되었다면, "***의 정보가 변경되었습니다" 문자열 반환.
	 * @return
	 */
	...
	public String removeStudent() throws InputMismatchException {
		...
		// 리스트에서 index 번 째 요소를 제거
		// 이 때, 제거된 요소가 반환된다.
		// * List 중간에 비어있는 인덱스가 없게 하기 위해
		//   remove() 동작 시, 뒤쪽 요소를 한 칸씩 알아서 당겨온다.
	...
	// String.contains("문자열") true / false
	/**
	 * 이름이 일치하는 학생을 찾아서 조회하는 메서드 (완전 일치)
	 * 검색할 이름 입력 받아 studentList에서 꺼내온 Student 객체의 name 값이 같은지 비교
	...
	public void searchName2() {
		...
		System.out.print("이름에 포함되는 문자 입력 : ");
		String input = sc.next();
		...
		for(Student std : studentList) {
			...
			// 순서 유의 (if문의 조건)
			// boolean String.contains(문자열) : String에 문자열이 포함되있으면 true / 없으면 false
...
//		Comparator<Student> nameComparator = Comparator.comparing(Student::getName).reversed();
		...
		//     기준으로 학생 객체를 비교하는 Comparator 객체를 생성.
...
/*	public void ex() {
		...
		// 반환형 : Object. List.get(index) : 리스트에서 index번째 인덱스에 있는 객체를 반환.
