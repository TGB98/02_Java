...
public class PolyService {
	...
	public void ex1() {
		...
		//*****다형성*****
		// 부모타입 참조변수 = 자식객체
		Car c2 = new Tesla(); // 에러 발생 x
		// Tesla 객체를 참조하는 변수의 타입이 Car(부모)이기 떄문에
		// Telsa 객체가 Car(부모) 객체로 변화함
		// - Up Casting 이라고 부름
		
		Car c3 = new Spark();
		// Spark 객체를 참조하는 변수(c3) 타입이 Car(부모)
		...
		c2.setFuel("전기");
		c2.setWheel(4);
	//	c2.setBatteryCapacity(1000000); 
		// 부모타입 참조변수기 때문에, Tesla의 고유필드, 메서드 사용 불가능
		...
		// 객체 배열 : 같은 객체 참조 자료형의 변수를 하나로 묶음으로 다루는 것
		// 다형성을 이용한 객체 배열
		...
		// 다형성(업 캐스팅) : 부모타입 참조변수 arr[i]로 자식 객체를 참조할 수 있다.	
		
	}
	
	
	// 매개변수로 받은 정수값에 따라 Car, Tesla, Spark 객체를 만들어
	// 반환하는 메서드
	...
	// 전달받은 Car 또는 자식객체(Tesla, Spark)의 엔진, 연료, 바퀴개수를 출력 메서드
	public void printCar(Car temp) {
		...
		// Car temp = s
		// 다형성의 업 캐스팅 모양과 똑같다
		// -> temp라는 매개변수에는 Tesla, Spark, Car의 주소가 넘어와도 된다(업 캐스팅)
...
//		Car[] arr = { new Car(), new Tesla(), new Spark() };
	...
	public void ex3() {
		// *****다형성 중 다운캐스팅*****
		// 다운 캐스팅이란
		// -> 부모타입 참조변수가 자식 객체를 참조하는 기술 ( 업 캐스팅 )
		// -> 적용된 상태에서만 진행할 수 있다.
		// 부모타입을 자식타입으로 "강제 형변환"해서
		// 자식객체의 본래 필드, 메서드를 사용 가능하게 하는 것
		...
		// 그래서 형변환 하는 부분을 () 로 한 번 묶어주고 나서 . 연산을 진행해야
		// Tesla 객체의 getBatteryCapacity() 를 사용할 수 있다
	...
	public void ex4() {
		...
		Car c1 = new Tesla(); // Tesla 객체를 만들어 Car 타입(부모타입) 참조변수 c1에 담음
