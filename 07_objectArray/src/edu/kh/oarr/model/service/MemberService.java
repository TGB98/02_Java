...
public class MemberService {
	...
	public MemberService() { // 기본생성자
		...
		memberArr[0] = new Member("user01","pass01","고길동",33,"서울");
		memberArr[1] = new Member("user02","pass02","둘리",100000000,"서울");
		memberArr[2] = new Member("user03","pass03","얼음별장군",999999,"얼음별");
	...
	public void displayMenu() {
		...
		public int emptyIndex() {
			...
			for(int i = 0; i < memberArr.length; i++) {
				...
				if(memberArr[i] == null) {
					return i;
					// 현재 메서드를 종료하고 호출한 곳으로 i값 가지고 돌아감
				}
			}
				...
				if(memberArr[i] != null) { // 회원정보가 있을 경우
					...
					if(memberArr[i].getMemberId().equals(memberId) &&
					   memberArr[i].getMemberPw().equals(memberPw) ) {
						...
						break; // 더 이상 같은 아이디, 비밀번호가 없기 때문에 효율을 위해 for문을 종료
					}
		...
		public void searchRegion() {
			...
			for(int i = 0; i < memberArr.length; i++) {
				...
				// 3) membeArr[i] 요소에 저장된 지역(region)이
		...
		public String selectMember() {
			...
			// 2) 로그인이 되있는 경우
			//  현재 로그인 한 회원의 정보를 출력할 문자열을 만들어 반환.
