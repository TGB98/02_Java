...
public class ByteService {
	...
	// IO 관련 코드 IOException이 발생될 가능성이 높다.
	// -> 거의 모든 메서드가 throws IOException
	// -> IOException 예외 처리 구문 작성이 필수.
	...
	public void fileByteOutput() {
		...
		try {
			...
			// -> 경로에 작성된 파일하고 연결된 파일 출력 스트림 객체 생성
			...
			sb.append("가나다라마바사아\n");
			sb.append("소드마스터 고길동\n");
			sb.append("SwordMaster Gogildong\n");
			sb.append("신짱구 신짱아 신형만 신봉선 흰둥이\n");
			

			
			// StringBuilder -> String 형 변환
			String content = sb.toString();
			
			// 출력방법 1: 한 글자(2byte)씩 파일로 출력
			// -> 2byte 범주의 문자(영어, 숫자, 기본 특수문자 제외)는 바이트 기반 스트림 통과 시
			// 데이터 손실이 발생해 글자가 깨지는 문제가 발생함.
			...
			System.out.println("[수행시간] : " + (endTime - startTime) + "ns"); // 62000ns
			
			
			fos.flush(); // 스트림 안에 남아있는 모든 데이터 모두 밀어냄.
		...
		finally { // try 예외 발생 여부 관계 없이 무조건 수행 구문
	...
	public void bufferedFileByteOutput() {
		 ...
		 * 
		 * 버퍼링
		 * - 인터넷 속도가 낮거나 스트리밍 코드 효율이 좋지 않을 때
		 ...
		 * 	출력용 보조 스트림  / 입력용 보조 스트림  -> 둘 다 바이트 기반 보조 스트림.
		 * 
		 * - FileOutputStream 은 1바이트씩 데이터를 입/출력
		 * -> 통로가 좁아서 데이터 입/출력이 느림.
		 * 
		 * -> 보조 스트림 이용 시, 버퍼에 입/출력할 내용을 모아서 한 번에 입/출력을 함.
		 * --> 기반 스트림을 이용하는 횟수가 적어짐.
		 * -> 성능, 시간 효율이 증가함.
		 * 
		 * */
		
		FileOutputStream fos = null; // 기반스트림 참조 변수 선언.
		
		BufferedOutputStream bos = null; // 보조스트림 참조 변수 선언.
		...
		try {
			...
			System.out.println("[수행시간] : " + (endTime - startTime) + "ns"); // 34200ns
			...
			try {
				...
				// 보조스트림(bos) close() 시
				// 보조스트림 생성에 사용된 기반 스트림(fos)도 같이 닫힘.
	...
	public void fileByteInput() {
		...
		try {
			...
			// InputStream 만들 때는 해당 경로에 실제 파일이 있어야 함.
			...
			while(true) {
				...
				// 더 이상 읽어올 값이 없으면 -1 반환.
				
				if(value == -1) break; // 다 읽어왔으면 반복 멈춤.
				
				sb.append( (char)value ); // char 형으로 강제 형변환 후 글자 형태로 sb에 추가.
				
			}
			System.out.println(sb.toString()); // String 형으로 변환 후 읽어온 내용 콘솔에 출력
	...
	public void fileCopy() {
		...
		try {
			...
			// 입력용 스트림을 이용해서 target 파일의 내용 입력 받기.
			byte[] bytes = bis.readAllBytes();
			...
			String copy = sb.toString(); // 복사할 파일의 경로 String 형으로 변환.
			...
			try {
				// 사용한 스트림 메모리 모두 반환.
				if(br != null) br.close();
				if(bis != null) bis.close(); // fis 같이 닫힘.
				if(bos != null) bos.close(); // fos 같이 닫힘.
