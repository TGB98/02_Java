...
public class FileService {
	 ...
	 * - File 클래스 메서드
	 *
	 * boolean  mkdir()          : 디렉토리 생성  ( mkdir = make directory )
	 * boolean  mkdirs()         : 경로상의 모든 디렉토리 생성 ( mkdirs = make directories )
	...
	public void method2() {
		...
		// Java에서 역슬래시(\)는 이스케이프 문자로 사용 됨.
		// -> \n 줄바꿈 , \t 탭 과 같이 특정 문자 뒤에 특별한 의미를 부여하는 역할.
		
		// -> 역슬래시 자체를 문자로 사용하고 싶으면 2개를 사용한다. -> \\
		...
		if( !file.exists() ) { // 존재하지 않으면
			...
			try {
				...
				// -> IOException은 반드시 예외 처리를 해야하는 Checked Exception
				// -> 예외 처리 필수
				if( file.createNewFile() ) { // 파일을 생성했다 , true 인 경우
					System.out.println(file.getName() + " 파일이 생성 되었습니다.");
				}
			} catch (IOException e) {
				e.printStackTrace(); // 예외 정보 + 예외가 발생한 위치 추적 메서드
			}
		}
...
//			System.out.println(date);
			...
			if(f.isDirectory()) {
				size = ""; // 폴더라면 사이즈 빈 문자열로 변경
				// -> 폴더 크기를 구하는 메서드는 따로 존재하지 않음.
				// -> 구하려면 폴더 내의 모든 파일을 순회하여 각 파일 크기를 합산.
			}
	...
	/*
	 * 스트림은 파일이나 데이터의 입/출력 작업을 위해 사용됨.
	 * 즉, 파일이나 폴더에 실제 데이터를 읽거나 쓸 때 스트림을 이용함.
	 * 하지만 폴더,파일 생성 및 삭제하는 것은 입/출력 작업이 아닌
	 * 파일 시스템과의 일반적인 상호작용이기 때문에 스트림 사용 할 필요가 없음.
	 * 
	 * */
