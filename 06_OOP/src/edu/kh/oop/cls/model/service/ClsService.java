...
public class ClsService /*extends Student*/{
	...
	public void ex1() {
		...
		// public class인 Student는 import 가능. (어디서든)
		
		
//		TestVO test = new TestVO();
		// (default) class인 TestVO는 import 불가능. (다른 패키지라 불가)
		
		
		// 필드 접근제한자 확인하기
		System.out.println( stu.v1 );
		// -> 전체에서 직접 접근 가능한 public인 v1만 에러발생 안함.
...
//		System.out.println( stu.v4 );
		...
		// Student 클래스를 상속받았으므로 
		// Student의 protected 변수인 v2는 직접 접근 가능
		//System.out.println( v3 ); // (default) 다른 패키지라 안됨.
		//System.out.println( v4 ); // 해당 클래스가 아닌 다른 클래스라 안됨.
	...
	public void ex2() {
		 ...
		 * static
		 * 
		 * 공유 메모리 영역(정적 메모리 영역) 이라고 한다.
		 * -> 프로그램 시작 시, static이 붙은 코드들이 모두
		 * static 영역에 생성되고
		 * 프로그램이 종료될 때까지 사라지지 않음. (정적)
		 * 그리고, static 영역에 생성된 변수는 어디서든지 공유 가능함.
		 * 
		 * */
	...
	public void ex4() {
		...
		User u3 = new User("user03","pass03","고길동",33,'남'); // 매개변수 5개 생성자( 전부 초기화 )
